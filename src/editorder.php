...
<?php
 ...
 foreach($result as $row){
    $txt.= '<tr><td>'.$row["order_id"].'</td><td>'.$row["user_id"].'</td><td>'.$row["status"].'</td><td>'.$row["total_amount"].'</td><td>'.$row["address"].'</td><td>'.$row["pincode"].'</td><td>'.$row["order_date"].'</td><td>'.$row["delivery_date"].'</td><td><input type="submit" class="btn btn-primary btn-sm" value="Edit"><input type="submit" class="btn btn-danger btn-sm" name="deleteProduct" value="Delete" id='.$row["order_id"].' onclick="Delete(this.id,2);"></td></tr>';
 }
 $txt.="</table>";
 echo $txt;
